...
class LoginModel
{
        ...
        if (password_verify($_POST['user_password'], $result->user_password_hash)) {
            ...
            if ($result->user_last_failed_login > 0) {
                $sql = "UPDATE users SET user_failed_logins = 0, user_last_failed_login = NULL
                        WHERE id = :id AND user_failed_logins != 0";
                $sth = $this->db->prepare($sql);
                $sth->execute(array(':id' => $result->id));
            }
            ...
            $sql = "UPDATE users SET user_last_login_timestamp = :user_last_login_timestamp WHERE id = :id";
            $sth = $this->db->prepare($sql);
            $sth->execute(array(':id' => $result->id, ':user_last_login_timestamp' => $user_last_login_timestamp));
            ...
            if (isset($_POST['user_rememberme'])) {
                ...
                $sql = "UPDATE users SET user_rememberme_token = :user_rememberme_token WHERE id = :id";
                $sth = $this->db->prepare($sql);
                $sth->execute(array(':user_rememberme_token' => $random_token_string, ':id' => $result->id));
    ...
    public function loginWithCookie()
    {
        ...
        $query = $this->db->prepare("SELECT id, user_name, user_email, user_password_hash, user_active,
                                          user_account_type,  user_has_avatar, user_failed_logins, user_last_failed_login
                                     FROM users
                                     WHERE id = :id
                                       AND user_rememberme_token = :user_rememberme_token
                                       AND user_rememberme_token IS NOT NULL");
        $query->execute(array(':id' => $id, ':user_rememberme_token' => $token));
...
//            $this->setGravatarImageUrl($result->user_email, AVATAR_SIZE);
            ...
            $sql = "UPDATE users SET user_last_login_timestamp = :user_last_login_timestamp WHERE id = :id";
            $sth = $this->db->prepare($sql);
            $sth->execute(array(':id' => $id, ':user_last_login_timestamp' => $user_last_login_timestamp));
    ...
    public function editUserName()
    {
        ...
        $query = $this->db->prepare("SELECT id FROM users WHERE user_name = :user_name");
        ...
        $query = $this->db->prepare("UPDATE users SET user_name = :user_name WHERE id = :id");
        $query->execute(array(':user_name' => $user_name, ':id' => $_SESSION['user_id']));
        $count =  $query->rowCount();
        if ($count == 1) {
    ...
    public function editUserEmail()
    {
        ...
        $query = $this->db->prepare("UPDATE users SET user_email = :user_email WHERE id = :id");
        $query->execute(array(':user_email' => $user_email, ':id' => $_SESSION['user_id']));
        $count =  $query->rowCount();
        if ($count != 1) {
            $_SESSION["feedback_negative"][] = FEEDBACK_UNKNOWN_ERROR;
            return false;
    ...
    public function registerNewUser()
    {
        ...
        if (empty($_POST['user_name'])) {
            ...
            $query = $this->db->prepare("SELECT id FROM users WHERE user_email = :user_email");
            ...
            $sql = "INSERT INTO users (user_name, user_password_hash, user_email, user_creation_timestamp, user_activation_hash, user_active)
                    VALUES (:user_name, :user_password_hash, :user_email, :user_creation_timestamp, :user_activation_hash, :user_active)";
...
//                $_SESSION["feedback_positive"][] = FEEDBACK_ACCOUNT_SUCCESSFULLY_CREATED;
            ...
            return true;
//            } else {
//                $query = $this->db->prepare("DELETE FROM users WHERE id = :last_inserted_id");
//                $query->execute(array(':last_inserted_id' => $id));
...
//            }
     ...
     */
    private function sendVerificationEmail($id, $user_email, $user_activation_hash)
    {
        ...
        $mail->Body = EMAIL_VERIFICATION_CONTENT . EMAIL_VERIFICATION_URL . '/' . urlencode($id) . '/' . urlencode($user_activation_hash);
     ...
     * @return bool success status
     */
    public function verifyNewUser($id, $user_activation_verification_code)
    {
        $sth = $this->db->prepare("UPDATE users
                                   SET user_active = 1, user_activation_hash = NULL
                                   WHERE id = :id AND user_activation_hash = :user_activation_hash");
        $sth->execute(array(':id' => $id, ':user_activation_hash' => $user_activation_verification_code));
    ...
    public function requestPasswordReset()
    {
        ...
        $query = $this->db->prepare("SELECT id, user_email FROM users
...
//        }
    ...
    public function changeAccountType()
    {
        if (isset($_POST["user_account_upgrade"]) AND !empty($_POST["user_account_upgrade"])) {
            ...
            $query = $this->db->prepare("UPDATE users SET user_account_type = 2 WHERE id = :id");
            ...
            $query = $this->db->prepare("UPDATE users SET user_account_type = 1 WHERE id = :id");
            $query->execute(array(':id' => $_SESSION["user_id"]));
